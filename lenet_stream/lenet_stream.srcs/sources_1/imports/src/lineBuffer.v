...
always@(*)begin
    if( pixel_finish_flag )begin
        if( data_out_flag_2 )begin
            dataValidOut                                                        <= 1'b1;
            for( i3 = 0; i3 < KERNEL_SIZE; i3 = i3 + 1'b1 )begin
                dataOut[ i3 * KERNEL_ROW_SIZE +: KERNEL_ROW_SIZE ]              <= line_buffer[ i3 ][ LINE_BUFFER_WIDTH - 1 -: KERNEL_ROW_SIZE ];
            end
        end
        else begin
            dataValidOut                                                        <= 1'b0;
        ...
        if( data_valid_in_reg )begin
            if( data_out_flag_1 & data_out_flag_2 )begin
                dataValidOut                                                    <= 1'b1;
                for( i4 = 0; i4 < KERNEL_SIZE; i4 = i4 + 1'b1 )begin
                    dataOut[ i4 * KERNEL_ROW_SIZE +: KERNEL_ROW_SIZE ]          <= line_buffer[ i4 ][ LINE_BUFFER_WIDTH - 1 -: KERNEL_ROW_SIZE ];
                end
            end
            else begin
                dataValidOut                                                    <= 1'b0;
